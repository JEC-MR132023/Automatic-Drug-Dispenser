...
class QRCodeScannerApp:
    ...
    def check_medicine_availability(self, patient_details):
            ...
            for medicine, quantity in medicines.items():
                ...
                self.cur.execute("SELECT id, stock FROM medicines WHERE name=?", (medicine,))
                result = self.cur.fetchone()
                if result:
                    medicine_id, stock = result
                    if stock >= quantity:
                        available_medicine_ids.append(str(medicine_id))
            return ", ".join(available_medicine_ids)
